...
export async function GET(request: NextRequest) {
  try {
    ...
    const users = result.rows.map((row: any) => {
      let deptPath = row.dept_path;
      if (deptPath) {
        deptPath = deptPath.replace(/^\d+_/, '').replace(/ > \d+_/g, ' > ');
        // (주)위엠비 접두사 제거 (공백 유무 상관없이 처리)
        deptPath = deptPath.replace(/^\(주\)\s*위엠비\s*>\s*/, '').replace(/^\(주\)\s*위엠비/, '');
      }

      let departmentName = row.department_name;
      if (departmentName) {
        departmentName = departmentName.replace(/^\(주\)\s*위엠비\s*>\s*/, '').replace(/^\(주\)\s*위엠비/, '');
      }

      return {
        ...row,
        department_name: departmentName,
        dept_path: deptPath, // Return the cleaned path
        roles: row.roles || []
      };
    });
