...
export interface MdEstimationTabsHandle {
  save: () => Promise<void>;
  deleteEstimation: () => void;
  complete: () => void;
  getStatus: () => string | undefined;
  getEstimationId: () => number | null;
}

const MdEstimationTabs = forwardRef<MdEstimationTabsHandle, { projectId: string; activeSubTab?: string; onStatusChange?: (status: string | undefined, estimationId: number | null) => void }>(
  ({ projectId, activeSubTab = "overview", onStatusChange }, ref) => {
    const id = projectId;
    ...
    const handleSave = useCallback(async () => {
      ...
      try {
        ...
        // VRB 상태 통합: MD 저장 시 부모에게 IN_PROGRESS 알림
        if (onStatusChange) {
          onStatusChange("IN_PROGRESS", estimationId);
        }
      } catch (error: any) {
      ...
      showAlert(
        ...
        async () => {
          ...
          try {
            // 먼저 저장 (데이터 보존)
            await handleSave();
            // VRB 상태 통합: MD 완료는 VRB 완료로 위임
            if (onStatusChange) {
              onStatusChange("COMPLETED", currentEstimationId);
            }
            showAlert("완료 처리되었습니다.", "success");
          } catch (error: any) {
            console.error("Error completing MD estimation:", error);
            showAlert(`완료 처리 실패: ${error.message}`, "error");
          } finally {
            setIsSaving(false);
          }
        }
      );
    }, [handleSave, currentEstimationId, onStatusChange, showAlert]);
    ...
    const handleCopyEstimation = useCallback(
      (estimationId: number) => {
        ...
        if (estimation) {
          ...
          handleNewEstimation();
          showAlert("기존 M/D 산정을 복사하여 새 버전을 생성합니다.", "info");
        }
      },
    ...
    // 외부에서 save/delete 호출 가능하도록 노출 (조건부 return 전에 선언)
    useImperativeHandle(ref, () => ({
      save: handleSave,
      deleteEstimation: handleDelete,
      getStatus: () => currentEstimation?.status,
      getEstimationId: () => currentEstimationId,
...
MdEstimationTabs.displayName = "MdEstimationTabs";
export default MdEstimationTabs;
