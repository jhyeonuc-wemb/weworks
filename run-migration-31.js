...
async function runMigration() {
    try {
        const migrationPath = path.join(process.cwd(), "database", "31_alter_vrb_add_hw_revenue_fields.sql");
        const sql = fs.readFileSync(migrationPath, "utf-8");
        console.log("Running migration 31...");
        await pool.query(sql);
        console.log("Migration 31 completed successfully.");
        await pool.end();
        process.exit(0);
    } catch (error) {
