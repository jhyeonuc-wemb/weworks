...
- **신규 생성/저장:** `variant="primary"`
- **취소/목록:** `variant="secondary"`
- **삭제:** `variant="danger"`
...
### 4.1 상태 드롭다운 (StatusDropdown)
- `@/components/ui/StatusDropdown.tsx` 사용
- 상세 페이지(MD, VRB, 수지분석, 정산)의 헤더에서 상태 변경 시 사용합니다.
- Badge와 유사한 색상 스키마를 사용하지만, 사용자 인터랙션을 통해 상태를 변경할 수 있습니다.
- **주요 기능:**
  - 현재 상태 표시 (Badge 스타일)
  - 드롭다운을 통한 상태 변경 (API 연동 포함)
...
모든 화면은 다양한 디바이스 환경에서 최적의 경험을 제공해야 합니다.

- **브레이크포인트 (Breakpoints):** Tailwind CSS 기본 브레이크포인트를 준수합니다.
  - `sm` (640px), `md` (768px), `lg` (1024px), `xl` (1280px), `2xl` (1536px)
- **전략:**
  - **Mobile-First:** 작은 화면부터 설계를 시작하여 큰 화면으로 확장해 나가는 방식을 권장합니다.
  - **Grid/Flex 활용:** 가변적인 레이아웃을 위해 CSS Grid와 Flexbox를 적극 활용합니다. (예: `grid-cols-1 md:grid-cols-2 lg:grid-cols-4`)
  - **최소/최대 너비:** 콘텐츠가 너무 퍼지거나 좁아지지 않도록 `max-w-screen-xl` 또는 `min-w-[320px]` 등의 제약 조건을 적절히 사용합니다.
  - **터치 타겟:** 모바일 환경을 고려하여 모든 클릭 요소는 최소 `h-10`(40px) 이상의 크기를 확보합니다.
- **표준 레이아웃 예시:**
  ```tsx
